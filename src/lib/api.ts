...
export const API_ENDPOINTS = [
  { method: "GET", path: "/health", purpose: "Liveness probe for the engine host", wired: true },
  {
    ...
    purpose: "Compiled investigation report + BSA s.63 evidentiary certificate",
    wired: true,
  },
  {
    method: "POST",
    path: "/sahyog/intake",
    purpose: "Simulated SAHYOG case intake — returns a job ID",
    wired: true,
  },
  {
    method: "POST",
    path: "/api/trace",
    purpose: "Trace a suspect address to its nearest VASP (planned)",
    wired: false,
  },
  {
    method: "GET",
    path: "/api/case/{case_id}",
    purpose: "Persisted case + graph payload (planned)",
    wired: false,
  },
  {
    method: "GET",
    path: "/api/registry/vasps",
    purpose: "VASP directory with FIU-IND and Travel Rule fields (planned)",
    wired: false,
  },
] as const;
