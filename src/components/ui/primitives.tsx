...
export function Field({ label, value, mono }: { label: string; value: ReactNode; mono?: boolean }) {
  return (
    <div className="min-w-0">
      ...
      <div
        className={cn("mt-1 text-sm break-words text-foreground", mono && "font-mono text-[12px]")}
      >
        {value}
      </div>
    </div>
...
export function DisclosureNote({
  ...
  const text = { warning: "text-warning", destructive: "text-destructive", info: "text-info" }[
    tone
  ];
  return (
